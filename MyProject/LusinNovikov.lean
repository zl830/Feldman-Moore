...
lemma union_mem_I {s t : Set α} (hs : s ∈ I f) (ht : t ∈ I f) :
  s ∪ t ∈ I f := by
  rw [I] at hs ht ⊢
  simp only [mem_setOf_eq, compl_union, inter_mem_iff]
  exact ⟨hs, ht⟩
...
lemma iUnion_mem_I (s : ℕ → Set α) :
  ...
  intro h
  rw[I] at h ⊢
  simp only [mem_setOf_eq, compl_iUnion] at h ⊢
...
lemma nonempty_of_nonnull (Fnonnull : ¬IsNull f F) : ∀ s ∈ F,
  ...
  rw[I]
  simp only [preimage_empty, inter_empty,
    mem_setOf_eq, compl_empty, univ_mem]
      ...
      · ext x
        ...
        · use u
          simp only [B', Finset.mem_union, Finset.mem_singleton, or_true,
            ite_true, Set.iUnion_true]
          exact h
        simp only [u1, Set.compl_iUnion, Set.mem_iInter, Set.mem_compl_iff,
          not_forall, not_not, exists_prop] at h
        rcases h with ⟨s, ⟨hsF, hs⟩⟩
        use s
        simp only [B', Finset.mem_union, Finset.mem_singleton,
      ...
      · apply MeasurableSet.inter
        · apply MeasurableSet.inter
          ...
          · apply measurableSet_preimage
            ...
            · apply (hBcover 0).2 (v 0)
              simp
            · apply (hBcover 0).2 (w 0)
              simp
        apply MeasurableSet.compl
      ...
      · intro xv hxv xw hxw
        ...
        have fxvU : f xv ∈ B' u := by
          simp only [B', u1, Set.compl_iUnion, ite_true, Set.mem_iInter,
            Set.mem_compl_iff]
          intro t ht
          simp only [B', u1, Set.compl_iUnion, ite_true,
            Set.mem_inter_iff, Set.mem_preimage, Set.mem_iInter,
            Set.mem_compl_iff] at hxv
          exact hxv.1.2 t ht
        rw[hB'u n] at fxvU
...
lemma null_of_decomp_null
  ...
  : IsNull f (F ∪ {u}) := by
      ...
      · ext x; constructor
        ...
        · rw[Set.mem_iUnion]
          ...
          have hsvne : s ≠ v n := by
            rw[hsw]
            exact (hvnew n).symm
          rw[hBeq, if_neg hsvne, if_pos hsw, Set.mem_inter_iff, ←hsw]
          exact ⟨hs, h⟩
      intro s hs
      rw[hBeq]
      split
    ...
    have hBdisjw : ∀ n : ℕ, ∀ s ∈ F, Disjoint (B n s) (B n (w n)) := by
      ...
      rw[Set.mem_iUnion]
      use hs
    exact null_of_decomp_null_with_common_disjoint_cover fmeas huborel huninF hdecomp
      hvborel hwborel hBdisjv hBdisjw hBcover hBnull hBcommon
...
variable [MetricSpace α] [sα : TopologicalSpace.SeparableSpace α]
variable [MetricSpace β] [sβ : TopologicalSpace.SeparableSpace β]
...
(u)² = ⋃ n : ℕ, v n ×ˢ w n. -/
omit mα sα in
lemma ediam_diam_aux {u : Set α} (ubdd : IsBounded u) : Metric.ediam u =
  ENNReal.ofReal (Metric.diam u) := by
    unfold Metric.diam
    exact (ENNReal.ofReal_toReal ubdd.ediam_ne_top).symm
...
lemma decomp_of_sq_minus_diag_u_real {u : Set α} {ε : ℝ} (uclosed : IsClosed u)
  ...
  := by
    ...
    have Usep : SeparableSpace u :=
      (IsSeparable.of_separableSpace u).separableSpace
    ...
    have hdecomp : ⋃ n : ℕ × ℕ, v n ×ˢ w n = sq_minus_diag u := by
      ...
      have : min (ε/2) (r/6) > 0 := by
        apply lt_min
        · simp only [Nat.ofNat_pos, div_pos_iff_of_pos_right, εpos]
      ...
      · exact hx2
      · constructor
        · exact le_trans (le_of_lt hb) (min_le_left _ _)
        · apply le_of_not_gt
          intro hdist
    ...
    · rename_i h
      have hnotmem : (D (Nat.unpair n).fst : α) ∉ u ∩ Metric.closedBall
...
lemma decomp_of_sq_minus_diag {u : Set α} {ε : ENNReal} (uclosed : IsClosed u)
  ...
  ∧ (∀ n : ℕ, (v n) ⊆ u) ∧ (∀ n : ℕ, (w n) ⊆ u)
  ∧ (∀ n : ℕ, IsClosed (v n)) ∧ (∀ n : ℕ, IsClosed (w n))
  ∧ (∀ n : ℕ, Metric.ediam (v n) ≤ ε)
  ∧ (∀ n : ℕ, Metric.ediam (w n) ≤ ε)
  ∧ (∀ n : ℕ, v n ≠ w n)
  := by
    by_cases h : ε = ⊤
    · choose v w hdecomp vsub wsub vclosed wclosed _ _ _ _ hne using
      ...
      refine ⟨v, w, ?_⟩
      rw[h]
      exact ⟨hdecomp, vsub, wsub, vclosed, wclosed,
        fun n => OrderTop.le_top (Metric.ediam (v n)),
        fun n => OrderTop.le_top (Metric.ediam (w n)), hne⟩
    · have hreal : ε.toReal > 0 := ENNReal.toReal_pos (ne_of_gt εpos) h
      choose v w hdecomp vsub wsub vclosed wclosed vbdd wbdd vdiam wdiam hne using
        decomp_of_sq_minus_diag_u_real uclosed unonempty hreal
      refine ⟨v, w, ?_⟩
      have vdiam' : ∀ n : ℕ, Metric.ediam (v n) ≤ ε := by
        intro n
        rw[ediam_diam_aux (vbdd n)]
        exact (ENNReal.ofReal_le_iff_le_toReal h).mpr (vdiam n)
      have wdiam' : ∀ n : ℕ, Metric.ediam (w n) ≤ ε := by
        intro n
        rw[ediam_diam_aux (wbdd n)]
...
lemma localization {ε : ENNReal} (εpos : ε > 0)
  ...
  := by
    ...
    · choose D Ddense using exists_dense_seq β
      let v (n : ℕ) := Metric.closedEBall (D n) (ε/2)
      use v
      constructor
      · ext x
        constructor
        ...
        · intro _
          ...
          have : 0 < (ε/2) := by
            exact ENNReal.div_pos (ne_of_gt εpos) ENNReal.ofNat_ne_top
          specialize Ddense x (ε/2) this
          rcases Ddense with ⟨a, ha⟩
          simp only [Set.mem_iUnion]
          use a
          exact le_of_lt ha
      · constructor
        · intro n
          apply le_trans Metric.ediam_closedEBall_le _
    ...
    · rw[not_nonempty_iff] at h
      let v (_ : ℕ) : Set β := ∅
      use v
      simp only [Set.iUnion_empty, Metric.ediam_empty, zero_le, forall_const,
        isClosed_discrete, and_self, and_true, v]
      symm
...
lemma splitting_one {H : P f} {u : Set α} (hu : u ∈ H.val) {ε : ENNReal}
  (εpos : ε > 0) : ∃ t1 t2, (IsClosed t1 ∧ t1 ⊆ u ∧ Metric.ediam t1 ≤ ε) ∧
  (IsClosed t2 ∧ t2 ⊆ u ∧ Metric.ediam t2 ≤ ε) ∧ Disjoint t1 t2 ∧
  ¬IsNull f (H.1 \ {u} ∪ {t1, t2}) := by
    ...
    have : ∃ n : ℕ, ¬IsNull f (H.1 \ {u} ∪ {v n, w n}) := by
      ...
      intro hcont; push Not at hcont; apply H.2.1
      ...
      have eq : H.1 \ {u} ∪ {u} = H.1 := by
        simp_all only [ne_eq, gt_iff_lt, not_true, mem_singleton, union_insert,
          mem_sdiff, not_and, not_not, and_false, not_false_eq_true,
          sdiff_union_self_eq_union, union_eq_left, singleton_subset_iff]
      rw[←eq]
      refine null_of_decomp_null (Continuous.measurable fcont)
        (IsClosed.measurableSet (H.2.2.1 u hu)) unin hdecomp vnin wnin hdisj ?_
        ?_ hcont
      · exact fun n => IsClosed.measurableSet (hvclosed n)
      · exact fun n => IsClosed.measurableSet (hwclosed n)
    rcases this with ⟨n, hn⟩
    refine ⟨v n, w n, ⟨hvclosed n, hvsub n, hvdiam n⟩,
      ⟨hwclosed n, hwsub n, hwdiam n⟩, ?_, hn⟩
...
lemma splitting_layer {H : P f} {Hs : Finset (Set α)} (hH : Hs ⊆ H.val)
  ...
  (∀ s ∈ G.val, s ∉ H.val → Metric.ediam s ≤ ε) := by
    classical
    set n := Hs.card with hn
    clear_value n
    induction n generalizing Hs
    · symm at hn
    ...
    have G'nonempty : ∀ s ∈ G', Set.Nonempty s := by
      intro s
      exact nonempty_of_nonnull hnull s
    ...
    · intro s hsH hsHs
      ...
      apply Or.inr
      apply Or.inl
      exact ⟨hGstay s hsH hsHs.2, hsHs.1⟩
    intro s hsG' hsH
...
small diameter -/
omit sα [BorelSpace α] in
lemma refine_layer (H : P f) {ε : ENNReal} (εpos : ε > 0)
  (hsmall : ∀ s ∈ H.val, Metric.ediam s ≤ ε) : ∃ G : P f,
  (∀ s ∈ H.val, ∃ t ∈ G.val, t ⊆ s) ∧ (∀ s ∈ G.val, Metric.ediam s ≤ ε) ∧
  Metric.ediam (f '' (⋃ s ∈ G.1, s)) ≤ ε := by
    classical
    choose v hvcover hvdiam hvclosed using localization (β := β) εpos
    have aux : ∃ n : ℕ, ¬IsNull f ((H.1).image fun s => s ∩ f⁻¹' (v n)) := by
      apply Classical.by_contradiction
      intro hcont; push Not at hcont
      apply H.2.1
    ...
    · simp only [H', ne_eq, Finset.mem_image, exists_exists_and_eq_and]
      intro s hs
      exact ⟨s, hs, Set.inter_subset_left⟩
    constructor
...
lemma splitting (H : P f) {ε : ENNReal} (εpos : ε > 0) : ∃ G : P f,
  ...
  Metric.ediam (f '' (⋃ s ∈ G.1, s)) ≤ ε := by
    ...
    refine ⟨G2, ?_, hG2indiam, hG2outdiam⟩
    intro s hs
    ...
    exact ⟨t'1, t'2, ⟨ht'1.1, subset_trans ht'1.2 ht1.2⟩,
      ⟨ht'2.1, subset_trans ht'2.2 ht2.2⟩,
      Set.disjoint_of_subset ht'1.2 ht'2.2 hne⟩
...
Continuous g ∧ Function.Injective g ∧ ∀ x y : ℕ → Bool, f (g x) = f (g y)
  ...
  let layer : ℕ → P f := fun n => by
    induction n with
    | zero => exact ⟨{Set.univ}, ⟨hnonnull, by simp⟩⟩
    | succ n ih => exact G ih (upos n.succ)
  ...
  have hdiam : VanishingDiam D := by
    ...
    · simp only [zero_le, Filter.eventually_atTop, implies_true, exists_const]
  ...
  have aux : ∃ n : ℕ, u n.succ ≤ ε := by
    rcases Filter.eventually_atTop.mp (hu.eventually_lt_const hε) with ⟨m, hm⟩
    exact ⟨m, (hm _ m.le_succ).le⟩
  obtain ⟨n, hn⟩ := aux
